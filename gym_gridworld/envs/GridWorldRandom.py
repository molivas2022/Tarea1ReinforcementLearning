...
class GridWorldRandom(gym.Env):
    metadata = {'render.modes': ['human']}

    def __init__(self, file_name = "map3.txt", fail_rate = 0.0, terminal_reward = 2.0, move_reward = 0.0, bump_reward = -0.5, bomb_reward = -1.5):
    ...
    def step(self, action):
        ...
        if rand_num < 0.15: #to the left
            if action == UP: action = LEFT
            elif action == LEFT: action = DOWN
            elif action == DOWN: action = RIGHT
            elif action == RIGHT: action = UP
        elif rand_num < 0.3: #to the right
            if action == UP: action = RIGHT
            elif action == LEFT: action = UP
            elif action == DOWN: action = LEFT
    ...
    def render(self, mode='human', close=False):
        ...
        if mode == 'human':
            ...
            self.viewer.window.set_size(800, 600)
